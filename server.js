...
mongoose.Promise = global.Promise;//before connect()
mongoose.connect('mongodb+srv://dzesika:<password>@cluster0-u5zqk.mongodb.net/test?retryWrites=true&w=majority', {
    useMongoClient: true
});
...
const userSchema = new Schema({
    ...
    created_at: Date,
    updated_at: Date
});//scheme, available values: string, number, date, boolean, array, mixed, objectId
...
userSchema.pre('save', function(next) {
    ...
    // next() goes to next hook następnego hooka to do before or after request
    next();
});
